...
| Pieza | Estado |
|---|---|
| `infra/` — Pulumi en TypeScript | **Existe**, y `yarn verificar` corre **sin Pulumi, sin token y sin clúster**. Hoy da **1 026 pruebas en 59 archivos, todas verdes**, con los cinco clones hermanos en la rama que este repositorio compone. Eran 902 con una roja declarada —`«stg» cabe`, que se cerró cuando el nodo creció—, 891 hasta la etapa 2 de #52, **840** con los cuatro sistemas y 726 hasta #21. **Su cuenta hay que leerla con los clones hermanos delante**, que no es un detalle y ya se pagó dos veces: el 2026-09-07 `main` llegó a tener **once rojas sin un solo commit propio** porque un hermano integró su interfaz, y el 2026-09-12 fueron **catorce** por lo mismo —`normativa`#41 y `catastro`#104 estrenaron las dos últimas—. Este flujo sólo se dispara con un `push` de aquí, así que esas rojas aparecen cuando alguien mira, no cuando se causan. Y los clones del disco pueden estar detrás de sus remotos —lo estuvieron 8, 1 y 9 commits— dando una cuenta que CI no reconoce ([`E` §6](docs/00-gobierno/E-el-monolito-sale-del-sistema.md)) |
...
abierto hasta la auditoría. Desde [#165](https://github.com/hneyra/infrastructure/issues/165)
