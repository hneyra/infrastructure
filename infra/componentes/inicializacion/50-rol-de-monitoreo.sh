...
# necesita; sin el, todo lo que puede consultar kamayuk_monitor es su propia sesion.
#
...
SELECT format('CREATE ROLE kamayuk_monitor LOGIN')
 WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'kamayuk_monitor') \gexec

ALTER ROLE kamayuk_monitor NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE NOREPLICATION
...
GRANT pg_monitor TO kamayuk_monitor;
SQL

# Se conecta a la base `postgres`, igual que kamayuk_respaldo (40-rol-de-respaldo.sh):
...
echo "Rol kamayuk_monitor listo: pg_monitor, nada de DDL."
