...
| 9 · restaurar `prod` | `--contra-cluster` se niega contra `prod` a propósito: es destructivo sobre el volumen en marcha. Lo que hay medido es `stg`, **restaurándose a sí mismo**. Nadie ha demostrado nunca que un respaldo de `prod` levante un motor, y el hueco está declarado —no tapado— en [el ensayo cruzado no existe](../../00-gobierno/el-ensayo-cruzado-no-existe.md) |
