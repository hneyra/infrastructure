...
`kamayuk_prueba_<uuid>` que `conMotorExterno` crea. Medido: `MotorPostgres.close()` las borra
...
habia una (`kamayuk_prueba_c86aff8b`), al terminar habia cero. La huella de los roles, en
cambio, sobrevive.
...
| M3 | El censo de bases `kamayuk_prueba_*` en vez de la huella de roles | **Rojo en falso**: 0 bases al terminar. Es por lo que la guarda no se escribio asi |
...
5. **No se toco ninguno de los cuatro sistemas.** Otro agente esta arreglando su flujo
   ...
   `kamayuk_prueba_`, y ya no: la guarda dejo de usarlo al cambiar de detector.
